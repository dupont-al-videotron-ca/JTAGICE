...
{
    internal sealed class ActivityClearEvents : ActivityProcessCommand
    {


        #region Constructors 
        public ActivityClearEvents(StructureActivity activityStructure, IActivityElement? parent) : base(activityStructure, parent!)
        {
        }
        #endregion
        ...
        public override bool OnReceivedResponse(ISlaveResponse response)
        {
            ...
            switch (response.ResponseId)
            {
                case SlaveResponseEnum.RSP_OK:
                    if (this.HasParent)
                    {
                        if (this.Parent is ActivitySetDeviceDescriptor)
                        {
                            _nextIndex = 0;
                            return true;
                        }
